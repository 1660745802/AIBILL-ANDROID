...
| `util/NotificationHelper.kt` | 删除 showConfirmNotification（不再逐条弹窗）；新增 showDailySummary() 每日汇总通知 |
...
| 2.2 | 每日 21:00 汇总通知（可选，用户可关） |
| 2.3 | 通知中心页增加"已自动入库"分区 |

### Phase 3（体验增强）

| 序号 | 任务 |
|------|------|
| 3.1 | AI 返回后自动触发学习引擎记忆商家→分类 |
| 3.2 | 学习引擎命中率统计展示 |
| 3.3 | 无障碍服务监听支付结果页（覆盖无通知场景） |
