...
┌─────────────────────────────────────────────────────────────────────────┐
│  信号源                                                                   │
│  NotificationListenerService (通知)  /  SmsReceiverService (短信)         │
└───────────────┬─────────────────────────────────────┬───────────────────┘
                │                                     │
                ▼                                     ▼
┌───────────────────────────────────────────────────────────────────────────┐
│ ① 预筛 (Source Filter)                                                    │
│   - 白名单匹配：NotificationSourceMapping.KNOWN_PACKAGES（45 家）          │
│   - 非白名单 → 丢弃                                                       │
└───────────────────────────────────┬───────────────────────────────────────┘
                                    │
                                    ▼
┌───────────────────────────────────────────────────────────────────────────┐
│ ② 正则解析 (NotificationParser / BankSmsPatterns)                         │
│   - 按 packageName 路由到对应正则集                                        │
│   - 提取：金额、收/支类型、商家/备注、订单号（如有）                          │
│   - 解析失败 → 转 ⑥ AI 兜底                                              │
└───────────────────────────────────┬───────────────────────────────────────┘
                                    │
                                    ▼
┌───────────────────────────────────────────────────────────────────────────┐
│ ③ 去重 (NotificationCorrelator)                                           │
│   三级优先级：                                                             │
│   P1: 订单号完全匹配 → 合并，保留信息最丰富的记录 (resolveConflict)          │
│   P2: 跨包名 + 同金额 + 10 秒窗口 → 合并（微信+银行同笔交易）              │
│   P3: 同金额 + 5 分钟窗口 → 标记疑似重复，弹窗确认                         │
│   - 通过则进入下一步；重复则丢弃或合并                                      │
└───────────────────────────────────┬───────────────────────────────────────┘
                                    │
                                    ▼
┌───────────────────────────────────────────────────────────────────────────┐
│ ④ 置信度评估 + 免确认判断 (AutoConfirmSuggester)                           │
│   - 关键词学习命中（用户历史确认过的商家关键词，30 天时效）→ 置信度 95       │
│     → 静默入库                                                            │
│   - 其他情况 → 弹窗确认（Heads-up Notification / App 内 Dialog）           │
│   - 注：已移除「小额免确认」策略（误判率高）                                │
└───────────────────────────────────┬───────────────────────────────────────┘
                                    │
                                    ▼
┌───────────────────────────────────────────────────────────────────────────┐
│ ⑤ 分类自动填充 (CategoryLearningEngine)                                   │
│   - 根据商家关键词 → 匹配历史分类映射 → 预填分类字段                        │
│   - 用户修改后反向更新映射关系（学习）                                      │
└───────────────────────────────────┬───────────────────────────────────────┘
                                    │
                                    ▼
...
| 原则 | 说明 |
|------|------|
| 准确率优先于召回率 | 宁可漏记（用户补录），不可误记（需要删除/修改） |
| 正则是预解析，不是判决 | 正则提取金额/商家等字段，但最终是否入库由置信度+用户确认决定 |
| 只有用户训练过的商家才静默入库 | AutoConfirmSuggester 仅当关键词命中用户历史学习记录时免确认 |
| 隐私保护 | AI 兜底预筛要求同时满足"有数字 + 有支付特征"才外发，避免泄漏聊天内容 |
| 用户可控 | 所有自动化行为（白名单/免打扰时段/免确认规则）均可在设置中关闭 |

### 11.3 置信度策略

| 场景 | 置信度 | 行为 |
|------|--------|------|
| 关键词学习命中（AutoConfirmSuggester 匹配 30 天内用户确认过的商家关键词） | 95 | 静默入库 + CategoryLearningEngine 预填分类 |
| 正则解析成功但无学习记录 | 60–80 | 弹窗确认（Heads-up / Dialog） |
| 正则解析失败，AI 兜底解析成功 | 50–70 | 弹窗确认 |
| AI 兜底也无法解析 | — | 存入 raw 记录，不打扰用户 |

### 11.4 分类自动填充逻辑 (CategoryLearningEngine)

```
用户确认一笔交易（商家="瑞幸咖啡"，分类="餐饮-饮品"）
    → CategoryLearningEngine 记录映射：关键词"瑞幸" → 分类"餐饮-饮品"
...
| 优先级 | 条件 | 窗口 | 行为 | 典型场景 |
|--------|------|------|------|----------|
| P1 | 订单号完全相同 | 无时间限制 | 合并，resolveConflict 保留字段最丰富的 | 微信通知 + 银行短信携带同一订单号 |
| P2 | 跨包名 + 同金额 | 10 秒 | 合并 | 支付宝付款 → 银行扣款通知 |
| P3 | 同金额 | 5 分钟 | 标记疑似重复，弹窗确认 | 不同渠道的延迟通知 |

> **已知问题**：P3 未排除同包名场景，若用户在同一 App 5 分钟内两次消费相同金额（如两杯 15 元咖啡），第二笔可能被误判为重复。后续考虑加 `packageName != record.packageName` 条件。

### 11.6 AI 兜底解析 + 二次校验

```
正则解析失败的通知文本
    → 预筛检查：同时包含数字 + 支付特征词（"付款/收款/消费/转账"等）
        ├── 不满足 → 丢弃，不外发（隐私保护）
        └── 满足 → 调用 AI 解析 API
                    → AiResultValidator 二次校验：
                        - 金额 > 0 且 ≤ MAX_AMOUNT_CENTS
                        - 类型为 income/expense
                        - 分类 ID 在合法范围
                        - 日期合理（不超过未来 1 天）
                    → 校验通过 → 弹窗确认
...
**NLS 心跳监测 (NlsHealthCheckWorker)**：
- WorkManager 定时执行（每 15 分钟）
- 检查 `Settings.Secure.enabled_notification_listeners` 是否包含本应用组件
- 权限丢失时发送本地通知引导用户重新授权

> **已知局限**：当前仅检查权限字符串，无法检测"权限在但服务被 OEM ROM 杀死"的情况。后续考虑增加绑定 ping 机制验证服务实际存活。

**通知撤回检测 (handleNotificationRemoved)**：
- 当系统回调 `onNotificationRemoved` 时，检查是否有对应的已解析记录
- 若已解析但未确认 → 将状态从 `parsed` 降级为 `raw`，防止误入账
- 典型场景：支付失败后 App 撤回了支付通知

### 11.8 白名单管理 (NotificationSourceMapping.KNOWN_PACKAGES)

当前白名单涵盖 **45 家**已知支付/银行类应用包名，包括：

- **支付平台**：微信 (`com.tencent.mm`)、支付宝 (`com.eg.android.AlipayGphone`)、云闪付、翼支付、美团支付等
- **银行类**：工行、建行、农行、中行、招行、交行、邮储、中信、浦发、民生、光大、华夏、兴业、平安等主流银行 App
- **电商/生活**：京东、淘宝、美团、饿了么、滴滴等（仅监听支付结果通知）
- **银行短信号**：95588（工行）、95533（建行）、95568（民生）等

白名单在 `NotificationSourceMapping` 中硬编码维护，后续可考虑云端下发更新。
